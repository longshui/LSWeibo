...
@interface LSComposeViewController ()<UITextViewDelegate, LSComposeToolbarDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
/** 输入控件 */
@property (nonatomic, weak) LSEmotionTextView *textView;
/** 键盘顶部的工具条 */
@property (nonatomic, weak) LSComposeToolbar *toolbar;
/** 相册（存放拍照或者相册中选择的图片） */
@property (nonatomic, weak) LSComposePhotosView *photoView;
/** 表情键盘 */
@property (nonatomic, strong) LSEmotionKeyboard *emotionKeyboard;
/** 是否正在切换键盘 */
@property (nonatomic, assign) BOOL switchingKeybaord;

@end

@implementation LSComposeViewController

- (LSEmotionKeyboard *)emotionKeyboard
{
    if (!_emotionKeyboard) {
        self.emotionKeyboard = [[LSEmotionKeyboard alloc] init];
...
-(void)setupPhotoView {
    ...
    LSComposePhotosView *photoView = [[LSComposePhotosView alloc]init];
...
{
    LSEmotion *emotion = notification.userInfo[@"SelectEmotionKey"];
...
-(void)composeToolbar:(LSComposeToolbar *)toolbar didClickButton:(LSComposeToolbarButtonType)buttonType {
    ...
    switch (buttonType) {
        case LSComposeToolbarButtonTypeCamera: // 拍照
            [self openCamera];
            break;
            
        case LSComposeToolbarButtonTypePicture: // 相册
            [self openAlbum];
            break;
            
        case LSComposeToolbarButtonTypeMention: // @
            LSLog(@"--- @");
            break;
            
        case LSComposeToolbarButtonTypeTrend: // #
            LSLog(@"--- #");
            break;
            
        case LSComposeToolbarButtonTypeEmotion: // 表情\键盘
...
- (void)sendWithImage
{
    ...
    params[@"access_token"] = [LSAccountManager shareAccountManager].account.access_token;
